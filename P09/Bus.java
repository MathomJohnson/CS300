...
public class Bus implements Comparable<Bus> {
  ...
  public int compareTo(Bus other) {
    //if other is null then this bus is greater
    if (other == null) return 1;
    //compare in the order of Arrival Time, Name, Direction
    if (this.getArrivalTime().compareTo(other.getArrivalTime()) > 0) return 1;
    if (this.getArrivalTime().compareTo(other.getArrivalTime()) < 0) return -1;
    if (this.getName().compareTo(other.getName()) > 0) return 1;
    if (this.getName().compareTo(other.getName()) < 0) return -1;
    if (this.route.getDirection().compareTo(other.route.getDirection()) > 0) return 1;
    if (this.route.getDirection().compareTo(other.route.getDirection()) < 0) return -1;
    return 0;//they are same if nothing else returns
  }
  ...
  public String toString() {
    return "[" + route.getRouteAndTripName() + " " + arrivalTime + " " + userStop + "]";
  }
}
